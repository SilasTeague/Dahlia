...
bool is_repetition_draw(const PositionHistory& history, const Position& pos, int ply) {
	...
	// and the halfmove clock counts exactly the plies since then -- so it is
	// both the correct scan limit and the reason this loop stays short.
	int scan = std::min(static_cast<int>(pos.halfmove_clock), history.size());

	// Entries at or after this index were pushed by the search; earlier ones
	// are moves actually played in the game.
	int root_index = history.size() - ply;

	int played_repeats = 0;
	// Only every other entry can match: the rest have the other side to move,
	// which the Zobrist key already distinguishes.
