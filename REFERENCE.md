...
In practice this means: hand-rolled sliding-piece attack generation over a pulled-in library (educational value + control) — starting with a simple loop/bit-shift ray walker, with magic bitboards as a deliberate, benchmarked future upgrade rather than a Milestone 1 requirement — a real benchmark harness before micro-optimizing (measurable engineering), and comments/docs that explain *why* a nonstandard chess-programming trick works, not just that it does.
...
│   ├── position/                (Position, Zobrist, make/unmake, repetition history)
...
**Decision (2026-08-14): the repetition history is a separate `PositionHistory` (`position/history.h`), not a member of `Position`.** A `Position` carries no past, so draws by repetition need the line that led to it — and most of the positions that matter for a threefold were reached before the search root, by moves the search never makes. Keeping that list of Zobrist keys *beside* `Position` rather than inside it keeps `make_move`/`unmake_move` free of push/pop bookkeeping: both run inside `generate_legal_moves`' legality filter and inside perft, neither of which can repeat anything, so maintaining the line there would tax the hottest path in the engine for nothing. Instead the UCI layer rebuilds the list from `position ... moves` (GUIs re-send the whole game every time), `think()` copies it, and `negamax` pushes/pops around its own make/unmake. `is_repetition_draw(history, pos, ply)` scans back at most `halfmove_clock` plies — nothing before the last pawn move or capture can recur — two at a time, since the intervening entries have the other side to move.
...
**Future extensions:** none outstanding. *`make_null_move`/`unmake_null_move` landed 2026-08-15 (Milestone 5).* The Zobrist rule the note below asked for: XOR out the en passant file if one is set (the right to capture en passant expires the moment its owner declines to use it, and leaving it would give two different positions the same key), XOR the side-to-move key, leave castling rights alone, advance the halfmove clock. No occupancy changes at all, which is what makes a null-move search cheap enough to be worth doing. `has_non_pawn_material(pos, color)` lives here too, as the pure position query behind the search's zugzwang guard.
...
5. Principal Variation Search (PVS): full window on the first move at each node, null-window (scout) search on the rest, re-search on fail-high. *Landed 2026-08-15 (Milestone 5).* Applied at every node with no depth threshold (one was tried and rejected — see the README's PVS section). The precondition was measured before the change rather than assumed: beta cutoffs land on the first move tried 84–96% of the time across the four benchmark positions at depth 7, which is what makes the scout the cheap question. Re-searches then run at 0.01–0.46% of scouts. Fail-soft is preserved, so the re-search condition is `alpha < score < beta`: inside a node that is itself being scouted, beta is already `alpha + 1`, and a fail-high there is the answer rather than a question to ask again. **PVS is exact but not score-identical in the presence of the TT** — a null-window search stores a *bound* where a full window stored an exact score, so a later probe may have to search where it used to return. Verified by rebuilding both versions with the TT's score cutoffs disabled: scores then match exactly at every depth and position tested.
...
8. Null-move pruning (with zugzwang guard — disable in low-material/pawn-only endgames where null move is unsound). *Landed 2026-08-15 (Milestone 5).* R = 2, minimum depth 3, never two in a row, never in check, and never when beta is already a mate score. The zugzwang guard is `has_non_pawn_material` (3.4) on the side to move: removing it and changing nothing else makes the search score the won K+P benchmark endgame at +219 instead of +962 at depth 17, which is what `tests/unit/test_nullmove.cpp` pins. Known limitation: the pruning runs before move generation, so a stalemate is treated as a position whose owner may pass — the guard covers the usual case, and detecting the rest would cost the move list this heuristic exists to skip.
9. Aspiration windows around the previous iteration's score. *Landed 2026-08-15 (Milestone 6).* Initial half-width 25 centipawns, applied from depth 4 up, skipped entirely when the previous score was a mate score. A failed window widens on the failing side only — a fail-low says nothing new about beta — and the new bound is taken from the returned score rather than from the old bound, which is what fail-soft buys: the search reports how far past the bound it got, so the next window is placed around the real answer. Widening is geometric (`delta += delta / 2`) so that a genuinely moved score costs a handful of re-searches rather than being chased outward one step at a time. **Aspiration is exact** — a score that escapes the window is never accepted, only re-searched wider — **but, like PVS above, it is not score-identical in the presence of the TT**, and for the same reason: a narrow window stores bounds where a wide one stored exact scores, so the table answers fewer questions later. That effect is not academic here, and it is the second time this project has met it: it moved the K+P benchmark endgame's win from depth 17 to depth 18, which `tests/unit/test_nullmove.cpp` records. Constants and the measurements behind them: `docs/adr/0006-aspiration-lmr-constants.md`.
10. Late move reductions (reduce search depth for late, quiet, non-critical moves in ordered move list; re-search at full depth on fail-high). *Landed 2026-08-15 (Milestone 6).* Reduction is `0.75 + ln(depth) · ln(move_index) / 2.25` truncated to an integer, from depth 3 up, after the first 3 moves at a node, clamped so a reduced search never falls below depth 1. Captures, promotions, moves that give check, and every move at a node that is in check are excluded. A reduced move that beats alpha is re-searched with the same null window at full depth, and only then, if it still beats alpha and the node has a real window, with the full window — three questions of increasing cost, so the expensive one is reached only by moves that survive both cheap ones. **This is the first technique in the engine that is allowed to be wrong**: a reduced move that fails low is believed without verification, which is the entire saving, so LMR can miss a good move — but it cannot promote a bad one, because beating alpha always triggers the full-depth re-search. That asymmetry is what `tests/unit/test_research.cpp` pins. Four conventional extra guards (zugzwang, killer-exempt, softer at PV nodes, four full-depth moves) were implemented and measured; all four were rejected, three of them as unresolvable by node counts alone rather than as harmful — see the ADR.
...
**Common pitfalls:** losing on time due to hard-limit checks being too infrequent; spending the *entire* allotted budget, so the reply arrives exactly on the deadline and everything after the search (output serialization, write, flush, transport) lands on the GUI's clock — GUIs with a zero time margin, which is cutechess-cli's default `timemargin`, score that as a forfeit, so a `Move Overhead` reserve is subtracted from every GUI-imposed budget including `movetime`; not accounting for `movestogo` in classical time controls; not handling `infinite`/`ponder` (even if `ponder` support itself is deferred, `go infinite` must be handled correctly since GUIs use it for analysis mode). `go infinite` needs an actual external `stop` to ever terminate, which in turn needs `go` to not block the thread reading `stop` off stdin — see docs/adr/0003-async-search-stop.md; before that ADR this pitfall was live (an "infinite" search could never be interrupted).
...
| **Elo, absolute** | What the engine is actually worth against real opposition | Rating on the Lichess ladder as a BOT account, over a stated number of games | Lichess (external) | The figure the README quotes. First recorded 2026-08-15: **1928 blitz** over 64 games (35W/6D/23L). A rating in motion, not a converged one — always quote it with the game count and date |
...
- **Goals:** correct, perft-verified pseudo-legal move generation. Fast comes later — magic bitboards are explicitly deferred (see [3.3](#33-move-generation-movegen)); this milestone's sliding-piece attacks are loop/bit-shift ray walks over live occupancy.
- **Deliverables:** `core/`, plain-struct `Move` (per [3.2](#32-move-representation-coremoveh)), precomputed knight/king/pawn attack tables, loop-based ray-walk sliding attacks (`rook_attacks`/`bishop_attacks`/`queen_attacks`), `movegen::generate_pseudo_legal_moves`.
- **Benchmarks:** loop-based sliding-attack lookup latency, move generation speed microbenchmarks committed as the first real `bench/results/` baseline — this baseline is what a future magic-bitboard swap gets measured against.
- **Tests:** perft against start position + Kiwipete + positions 3–6, to at least depth 5–6 in CI (deeper locally/manually).
- **Success criteria:** perft matches reference values exactly at all tested depths; loop-based sliding-attack benchmark recorded as baseline.
...
**Status (2026-08-15).** Complete, apart from the SPRT match — which, as at Milestone 4, is not an engine deliverable and lives in a separate repository. Against the Milestone 4 tag, nodes to depth 7 fell 22.5% (opening), 53.5% (Kiwipete) and 91.5% (tactical), and depth reached in a fixed 5 s rose from 10 to 11, 8 to 10, and 8 to 11 respectively. The Win At Chess subset went 14/18 → 15/18.

The K+P endgame is the milestone's honest cost: it *lost* three plies in fixed time (27 → 24), two to the evaluation becoming a full board walk and one to PVS, of which null-move pruning could return only one — the zugzwang guard correctly refuses to prune there at all. Its node count at depth 5 is unchanged to the node by null-move pruning, which is the guard being visible in the numbers rather than only in the code.

The detail below is kept as the record of how the milestone went.

**Progress note (2026-08-15).** Tapered piece-square tables landed first, ahead of the search work, because the tactics suite's known-failure list said evaluation — not search — was the binding constraint (see Milestone 4's status). It solved one of the four recorded failures (Win At Chess 14/18 → 15/18 at depth 7); the three that remain need king safety or the bishop pair, which PSTs structurally cannot express. It also cost 25–33% of nodes/sec and a ply of depth in fixed time, recorded rather than smoothed over: eval went from ten popcounts to a full board walk, and the remaining deliverables shrink the tree the eval now walks.

PVS landed second and took 3–21% of the nodes back at fixed depth without changing a single score or best move — those are now pinned alongside the node counts in `tests/unit/test_search_nodes.cpp`, so an efficiency change and a change of mind fail different tests. The K+P endgame is the exception in both directions: it lost two plies of depth in a fixed five seconds, traced to TT bound-vs-exact information rather than to re-search overhead (measured at 0.01–0.46% of scouts), and documented rather than tuned away after two candidate fixes were measured and rejected.

Null-move pruning landed third and is the largest single-change node reduction in the project so far: −41% (opening), −53% (Kiwipete) and −92% (tactical) at depth 7, again with every score and best move unchanged.

Both tests this milestone asked for exist: the eval-symmetry test is `test_eval.cpp`'s colour-swapped mirror test, and the zugzwang regression test is `test_nullmove.cpp`, which pins the 700-centipawn misjudgement a guard-less build makes in the K+P benchmark position.
...
The replacement is not weaker, it is real. **Dahlia now plays rated games on Lichess as a BOT account**, which is a stronger measurement than the SPRT match originally specified, and in one respect a different kind: SPRT compares two Dahlia versions against each other and yields a *relative* Elo delta, while the Lichess ladder gives an *absolute* rating against a pool of humans and other engines. The relative comparison remains the better instrument for small changes and stays available for tuning work; the absolute figure is what the README needs and what the portfolio goal actually asks for.

**Baseline recorded 2026-08-15, 10:43 EST:** Dahlia is rated **1928 blitz** over 64 games — 35 wins, 6 draws, 23 losses, a 59.4% score. This is the figure Milestone 6's gain is measured against, and it is the first Elo number the project has ever had. Note what it is not: it is a rating in motion over a small sample, not a converged one, and the milestone's own changes are part of what moves it.
...
| 2026-08-15 | The tactics suite's depth rises to 10 and its solve count stops being compared across milestones; LMR makes a nominal ply mean less tree, so the suite pins *which* positions fail, not how many at depth 7 | See Milestone 6 | — (settled here, no separate ADR needed) |
| 2026-08-15 | Null-move pruning uses R=2 at depth ≥ 3 with `has_non_pawn_material` as the zugzwang guard; stalemate misjudgement is a known, accepted limitation rather than something to pay move generation to avoid | See 3.4, 3.8 | — (settled here, no separate ADR needed) |
| 2026-08-15 | PVS is applied at every node with no depth threshold, and the K+P endgame's node regression is documented rather than tuned around; a depth threshold and an exact-preferring TT replacement rule were both measured and rejected | See 3.8, README | — (settled here, no separate ADR needed) |
